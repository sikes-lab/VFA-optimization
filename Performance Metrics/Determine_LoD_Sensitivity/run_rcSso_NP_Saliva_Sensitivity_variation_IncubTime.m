rcSso_NP_Swab_Sensitivity_variation_IncubTime(0.2) %Measure Sensitivity @ Cyan Intensity of 0.2

function rcSso_NP_Swab_Sensitivity_variation_IncubTime(Sensitivity_Location)
...
Sens_All = zeros(size(IncubTimes));
Sens_Loc = zeros(size(IncubTimes));
...
plot(IncubTimes,Sens_All,'LineWidth',1.5)
xlabel('Incubation Time (s)'); ylabel('Sensitivity (AU\/nM)');
ax = gca;
ax.FontSize = 12;
axis tight;
...
saveas(gcf,'rcSso_NP_Saliva_vary_IncubTime_Sensitivity_changes.fig')
save('Change_IncubTime_Sens_Saliva.mat','IncubTimes','Sens_All');
