rcSso_NP_Saliva_SN_variation_HRP_Conc(5e-9.*1e3)

function rcSso_NP_Saliva_SN_variation_HRP_Conc(Probe_Conc)
...
% No longer needed
% S_All = sum(S_All,1); %Note that S_All is a row vector
% N_All = sum(N_All,1);
...
yyaxis left;
plot(HRP_Concs,SoverN,'o','Color','b','LineWidth',1.5); 
ylabel('S/N Ratio');
yyaxis right;
plot(HRP_Concs,SminusN,'x','Color','r','LineWidth',1.5);
ylabel('(S-N) (AU)');
xlabel('[SA-HRP] (ng/mL)');
...
saveas(gcf,'rcSso_NP_Saliva_vary_HRP_SN_changes.fig')
save('Change_HRP_Conc_Saliva_SN.mat','HRP_Concs','S_All','N_All','SoverN','SminusN');
