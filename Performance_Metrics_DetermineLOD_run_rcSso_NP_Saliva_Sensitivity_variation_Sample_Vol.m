rcSso_NP_Saliva_Sensitivity_variation_Sample_Vol(0.2) %Measure Sensitivity @ Cyan Intensity of 0.2

function rcSso_NP_Saliva_Sensitivity_variation_Sample_Vol(Sensitivity_Location)
...
Sample_Vols = linspace(5,45,51); %[uL]
...
Soln_pptys_in = repmat(Soln_pptys,length(Sample_Vols),1);
...
Paper_params_in = repmat(Paper_params,length(Sample_Vols),1);

op_params_in = cell(length(Sample_Vols),3);
...
Sens_All = zeros(size(Sample_Vols));
Sens_Loc = zeros(size(Sample_Vols));

MW_HRP = Soln_pptys_in(1,3)*1000; %pick a random one to use

beta_conc1 = beta_conc(1);
beta_conc2 = beta_conc(2);
for i = 1:length(Sample_Vols)    
    op_params_in{i,2} = [Sample_Vols(i) 50].*(1e-6)*1e-3; %[m3] X uL sample + 50 (25+25) uL wash
end

parfor i = 1:length(Sample_Vols)    
...
plot(Sample_Vols,Sens_All,'LineWidth',1.5)
xlabel('Sample Volume (\muL)'); ylabel('Sensitivity (AU/nM)');
ax = gca;
ax.FontSize = 12;
axis tight;
...
saveas(gcf,'rcSso_NP_Saliva_vary_SampleVol_Sensitivity_changes.fig')
save('Change_Sample_Vols_Sens_Saliva.mat','Sample_Vols','Sens_All');
