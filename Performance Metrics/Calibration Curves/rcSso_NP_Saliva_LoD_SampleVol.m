...
all_results = zeros(length(conc_plot),2,length(SampleVols)); %to hold all results

figure;
for j = 1:length(SampleVols)    
    % Intialize
    Cyan_HRP_model_pos = zeros(size(conc_plot));
    Cyan_HRP_model_neg = zeros(size(conc_plot));
    op_params{1,2} = [SampleVols(j) 50].*(1e-6)*1e-3; %[m3] X uL sample + 50 (25+25) uL wash
    [op_params_in{:,2}] = deal(op_params{:,2});
...
for j = 1:length(SampleVols)
    Positives = [Positives semilogx(conc_plot, all_results(:,2,j),'LineWidth',1.5,'Color',[0 (1-j/length(SampleVols)).*1 1])]; 
    hold on;
    Background = [Background semilogx(conc_plot, all_results(1,1,j).*ones(size(conc_plot)),'--','LineWidth',1.5,'Color',[1 0 (1-j/length(SampleVols)).*1])];       
    Negatives = [Negatives semilogx(conc_plot, all_results(:,1,j),'LineWidth',1.5,'Color',[1 0 (1-j/length(SampleVols)).*1])]; 
    legendlabel = [legendlabel convertCharsToStrings(strcat(num2str(SampleVols(j))))]; 
end
...
title(lgd,{'\color{blue}Positive\color{black}&\color{red}Background \color{black} Signal','with Sample Volume (\muL)'})

title('Variation of LoD with Sample Volume'); box on;
...
saveas(gcf,'rcSso_NP_Saliva_vary_SampleVol_LoD_Range.fig')
save('Change_SampleVol_Saliva_Range.mat','conc_plot','all_results');
