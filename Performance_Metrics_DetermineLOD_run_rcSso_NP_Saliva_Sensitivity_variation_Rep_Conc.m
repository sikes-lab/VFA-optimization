rcSso_NP_Swab_Sensitivity_variation_Rep_Conc(0.2) %Measure Sensitivity @ Cyan Intensity of 0.2

function rcSso_NP_Swab_Sensitivity_variation_Rep_Conc(Sensitivity_Location)
...
Soln_pptys_in(:,8) = (Rep_Concs.').*1e-9.*1e3; %make matrix early.
...
Sens_All = zeros(size(Rep_Concs));
Sens_Loc = zeros(size(Rep_Concs));
...
plot(Rep_Concs,Sens_All,'LineWidth',1.5)
xlabel('[Reporter] (nM)'); ylabel('Sensitivity (AU/nM)');
ax = gca;
ax.FontSize = 12;
axis tight;
...
saveas(gcf,'rcSso_NP_Saliva_vary_Rep_Sensitivity_changes.fig')
save('Change_Rep_Conc_Sens_Saliva.mat','Rep_Concs','Sens_All');
