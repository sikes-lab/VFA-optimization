...
k_offs = [4 2 1 0.5 0.25].*(1e-3); %[1/s]
conc_plot = logspace(-2,4,101); %Conc range for probing (1 fM to 300 nM)
...
all_results = zeros(length(conc_plot),2,length(k_offs)); %to hold all results

figure;
for j = 1:length(k_offs)    
    % Intialize
    Cyan_HRP_model_pos = zeros(size(conc_plot));
    Cyan_HRP_model_neg = zeros(size(conc_plot));
    Soln_pptys(7) = k_offs(j); %[m3/mol.s] association rate constants
...
for j = 1:length(k_offs)
    Positives = [Positives semilogx(conc_plot, all_results(:,2,j),'LineWidth',1.5,'Color',[0 (1-j/length(k_offs)).*1 1])];hold on; 
    Background = [Background semilogx(conc_plot, all_results(1,1,j).*ones(size(conc_plot)),'LineWidth',1.5,'Color',[1 0 (1-j/length(k_offs)).*1])]; 
    Negatives = [Negatives semilogx(conc_plot, all_results(:,1,j),'LineWidth',1.5,'Color',[1 0 (1-j/length(k_offs)).*1])]; 
    legendlabel = [legendlabel convertCharsToStrings(strcat(num2str(k_offs(j).*1e4)))];
end

hold off
% Pos_lgd = legend(ax1,Positives,legendlabel,'Location','NorthEastOutside');
% ax2 = copyobj( ax1, gcf);
% Neg_lgd = legend(ax2,Negatives,legendlabel,'Location','SouthEastOutside');
% title(Pos_lgd,{'Positive Signal','[NA-HRP] ='})
% title(Neg_lgd,{'Negative Signal','[NA-HRP] ='})

lgd = legend([Positives Negatives],[legendlabel legendlabel],'NumColumns',2);

title(lgd,{'\color{blue}Positive\color{black}&\color{red}Background \color{black} Signal','with k_{off}(\times 10^{-4} s^{-1})'})


title('Variation of LoD with k_{off}'); box on;
...
saveas(gcf,'rcSso_NP_Saliva_vary_k_off_LoD_Range.fig')
save('Change_k_on_Saliva_Range.mat','conc_plot','all_results');

% %Also save one on loglog axis
% set(gca,'YScale','log')
% ylim([1e-3 inf])
% 
% saveas(gcf,'rcSso_NP_Swab_vary_k_off_LoD_loglog.fig')
% 
