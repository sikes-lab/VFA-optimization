...
all_results = zeros(length(conc_plot),2,length(HRP_Concs)); %to hold all results

figure;
for j = 1:length(HRP_Concs)    
    % Intialize
    Cyan_HRP_model_pos = zeros(size(conc_plot));
    Cyan_HRP_model_neg = zeros(size(conc_plot));
    Soln_pptys(9) = (HRP_Concs(j).*1e-9.*1e6)./(Soln_pptys(3).*1e3); %[mol/m3] Used SA-HRP in ng/mL in Sample solution
...
for j = 1:length(HRP_Concs)
    Positives = [Positives semilogx(conc_plot, all_results(:,2,j),'LineWidth',1.5,'Color',[0 (1-j/length(HRP_Concs)).*1 1])]; 
    hold on;
    Background = [Background semilogx(conc_plot, all_results(1,1,j).*ones(size(conc_plot)),'--','LineWidth',1.5,'Color',[1 0 (1-j/length(HRP_Concs)).*1])];       
    Negatives = [Negatives semilogx(conc_plot, all_results(:,1,j),'LineWidth',1.5,'Color',[1 0 (1-j/length(HRP_Concs)).*1])]; 
    legendlabel = [legendlabel convertCharsToStrings(num2str(HRP_Concs(j)))]; %Use actual concentration
end
...
title(lgd,{'\color{blue}Positive\color{black}&\color{red}Background \color{black} Signal','with Label Conc (ng/mL)'})

title('Variation of LoD with HRP Concentration'); box on;
...
saveas(gcf,'rcSso_NP_Saliva_vary_HRP_LoD_Range.fig')
save('Change_HRP_Conc_Saliva_Range.mat','conc_plot','all_results');
