...
Conc_hold = Sorted_NP_Saliva_Data.Rep_Conc.Neg((Sorted_NP_Saliva_Data.Rep_Conc.Neg(:,2) == 1),1); % was in X

intensity_hold = Sorted_NP_Saliva_Data.Rep_Conc.Neg((Sorted_NP_Saliva_Data.Rep_Conc.Neg(:,2) == 1),7:8);
avg1min_Cyan_neg_Cyan = [Conc_hold intensity_hold];

% Extract for positive
Conc_hold = Sorted_NP_Saliva_Data.Rep_Conc.Pos((Sorted_NP_Saliva_Data.Rep_Conc.Pos(:,2) == 1),1);
intensity_hold = Sorted_NP_Saliva_Data.Rep_Conc.Pos((Sorted_NP_Saliva_Data.Rep_Conc.Pos(:,2) == 1),7:8);
avg1min_Cyan_Sso_Cyan = [Conc_hold intensity_hold];
...
Rep_Concs_plot = linspace(5,205,51); % [nM] Probe from 5 nM to 205 nM
...
Paper_params_in = repmat(Paper_params,length(Rep_Concs_plot),1);

Soln_pptys_in = repmat(Soln_pptys,length(Rep_Concs_plot),1);
Soln_pptys_in(:,8) = (Rep_Concs_plot.').*1e-9.*1e3;

op_params_in = cell(length(Rep_Concs_plot),3);
...
% Intialize
Cyan_HRP_model_pos = zeros(size(Rep_Concs_plot));
Cyan_HRP_model_neg = zeros(size(Rep_Concs_plot));

Sample_Conc = 5e-9*1e3; % 5nM NP used as baseline.

MW_HRP = Soln_pptys(3)*1000;
parfor i = 1:length(Rep_Concs_plot)
...
figure;
plot(Rep_Concs_plot, Cyan_HRP_model_pos,'LineWidth',1.5); 
hold on;
...
plot(Rep_Concs_plot, Cyan_HRP_model_neg,'LineWidth',1.5); %convert to uL/s, plot in red, Cyan express in percent
...
title('Influence of Reporter conc on Signal generated'); box on;
xlabel('[Reporter] (nM)'); ylabel('Cyan Intensity (AU)');
...
saveas(gcf,'rcSso_NP_Saliva_vary_RepConc_plot.fig')
