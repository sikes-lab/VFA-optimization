...
k_ons = [0.25 0.5 1 2 4].*(2e5).*1e-3; %[m3/mol.s]
...
all_results = zeros(length(conc_plot),2,length(k_ons)); %to hold all results

figure;
for j = 1:length(k_ons)    
    % Intialize
    Cyan_HRP_model_pos = zeros(size(conc_plot));
    Cyan_HRP_model_neg = zeros(size(conc_plot));
    Soln_pptys(6) = k_ons(j); %[m3/mol.s] association rate constants
...
for j = 1:length(k_ons)
    Positives = [Positives semilogx(conc_plot, all_results(:,2,j),'LineWidth',1.5,'Color',[0 (1-j/length(k_ons)).*1 1])];hold on; 
    Background = [Background semilogx(conc_plot, all_results(1,1,j).*ones(size(conc_plot)),'--','LineWidth',1.5,'Color',[1 0 (1-j/length(k_ons)).*1])]; 
    Negatives = [Negatives semilogx(conc_plot, all_results(:,1,j),'LineWidth',1.5,'Color',[1 0 (1-j/length(k_ons)).*1])]; 
    legendlabel = [legendlabel convertCharsToStrings(num2str(k_ons(j).*1e-2))];
end
...
title(lgd,{'\color{blue}Positive\color{black}&\color{red}Background \color{black} Signal','with k_{on} (\times 10^{5} M^{-1}s^{-1})'})


title('Variation of LoD with k_{on}'); box on;
...
saveas(gcf,'rcSso_NP_Saliva_vary_k_on_LoD_Range.fig')
...
% ylim([1e-3 inf])
% 
% saveas(gcf,'rcSso_NP_Swab_vary_k_on_LoD_loglog.fig')
% 
