rcSso_NP_Saliva_SN_variation_k_on(5e-9.*1e3)

function rcSso_NP_Saliva_SN_variation_k_on(Probe_Conc)
...
yyaxis left;
plot(k_ons,SoverN,'Color','b','LineWidth',1.5); 
ylabel('S/N Ratio');
yyaxis right;
plot(k_ons,SminusN,'Color','r','LineWidth',1.5);
ylabel('(S-N) (AU)');
xlabel('k_{on} (M^{-1}s^{-1})');
...
saveas(gcf,'rcSso_NP_Saliva_vary_k_on_SN_changes.fig')
save('Change_k_on_Saliva_SN.mat','k_ons','S_All','N_All','SoverN','SminusN');
