rcSso_NP_Swab_SN_variation_IncubTime(5e-9.*1e3)

function rcSso_NP_Swab_SN_variation_IncubTime(Probe_Conc)
...
IncubTimes = linspace(1,120,51); %[s]
...
Soln_pptys_in = repmat(Soln_pptys,length(IncubTimes),1);
...
Paper_params_in = repmat(Paper_params,length(IncubTimes),1);

op_params_in = cell(length(IncubTimes),3);
...
for i = 1:length(IncubTimes)    
    op_params_in{i,5} = [1*60 IncubTimes(i)]; %[s]
end

MW_HRP = Soln_pptys(3)*1000; 
S_All = zeros(size(IncubTimes));

parfor i = 1:length(IncubTimes)    
    S_All(i) = sum(MW_HRP.*1e12.*Premix_VFA_parfor_nHRP_only_3_Stoichio_by10(Probe_Conc,Soln_pptys_in,Paper_params_in,op_params_in,M,N,InletBC,i));
    N_All(i) = sum(MW_HRP.*1e12.*Premix_VFA_parfor_nHRP_only_3_Stoichio_by10(0,Soln_pptys_in,Paper_params_in,op_params_in,M,N,InletBC,i));
end
...
yyaxis left;
plot(IncubTimes,SoverN,'Color','b','LineWidth',1.5); 
ylabel('S/N Ratio');
yyaxis right;
plot(IncubTimes,SminusN,'Color','r','LineWidth',1.5);
ylabel('(S-N) (AU)');
...
saveas(gcf,'rcSso_NP_Saliva_vary_IncubTime_SN_changes.fig')
save('Change_IncubTime_Saliva_SN.mat','IncubTimes','S_All','N_All','SoverN','SminusN');
