...
Incub_Times = [5 12.5 25.5 40 60]; %[s]
...
all_results = zeros(length(conc_plot),2,length(Incub_Times)); %to hold all results

figure;
for j = 1:length(Incub_Times)    
    % Intialize
    Cyan_HRP_model_pos = zeros(size(conc_plot));
    Cyan_HRP_model_neg = zeros(size(conc_plot));
    op_params{1,5} = [1*60 Incub_Times(j)]; %Incubated premix for 15 min, followed by some time with sample
    [op_params_in{:,5}] = deal(op_params{:,5});
...
for j = 1:length(Incub_Times)
    Positives = [Positives semilogx(conc_plot, all_results(:,2,j),'LineWidth',1.5,'Color',[0 (1-j/length(Incub_Times)).*1 1])]; 
    hold on;
    Background = [Background semilogx(conc_plot, all_results(1,1,j).*ones(size(conc_plot)),'--','LineWidth',1.5,'Color',[1 0 (1-j/length(Incub_Times)).*1])];       
    Negatives = [Negatives semilogx(conc_plot, all_results(:,1,j),'LineWidth',1.5,'Color',[1 0 (1-j/length(Incub_Times)).*1])]; 
    legendlabel = [legendlabel convertCharsToStrings(strcat(num2str(Incub_Times(j))))]; 
end
...
title(lgd,{'\color{blue}Positive\color{black}&\color{red}Background \color{black} Signal','with Incubation Time (s)'})

title('Variation of LoD with Incubation Time'); box on;
...
saveas(gcf,'rcSso_NP_Saliva_vary_IncubTime_LoD_Range.fig')
save('Change_IncubTime_Saliva_Range.mat','conc_plot','all_results');
