rcSso_NP_Swab_Sensitivity_variation_k_on(0.2) %Measure Sensitivity @ Cyan Intensity of 0.2

function rcSso_NP_Swab_Sensitivity_variation_k_on(Sensitivity_Location)
...
Sens_All = zeros(size(k_ons));
Sens_Loc = zeros(size(k_ons));
...
plot(k_ons,Sens_All,'LineWidth',1.5)
xlabel('k_{on} (M^{-1}s^{-1})'); ylabel('Sensitivity (AU/nM)');
ax = gca;
ax.FontSize = 12;
axis tight;
...
saveas(gcf,'rcSso_NP_Saliva_vary_k_on_Sensitivity_changes.fig')
save('Change_k_on_Sens_Saliva.mat','k_ons','Sens_All');
