rcSso_NP_Saliva_Sensitivity_variation_HRP_Conc(0.2) %Measure Sensitivity @ Cyan Intensity of 0.2

function rcSso_NP_Saliva_Sensitivity_variation_HRP_Conc(Sensitivity_Location)
...
Sens_All = zeros(size(HRP_Concs));
Sens_Loc = zeros(size(HRP_Concs));
...
%HRP_Concs=HRP_Concs.*1e-6.*1e6/((Soln_pptys(3))*1e3)./15;

%Sens_All = Sens_All; %Convert to /(ug/mL)
plot(HRP_Concs,Sens_All,'LineWidth',1.5)
xlabel('[SA-HRP] (ng/mL)'); ylabel('Sensitivity (AU/nM)');
ax = gca;
ax.FontSize = 12;
axis tight;
...
saveas(gcf,'rcSso_NP_Saliva_vary_HRP_Sensitivity_changes.fig')
save('Change_HRP_Conc_Sens_Saliva.mat','HRP_Concs','Sens_All');
