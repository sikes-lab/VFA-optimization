...
Dilution_hold = Sorted_NP_Saliva_Data.SampleVol.Neg((Sorted_NP_Saliva_Data.SampleVol.Neg(:,2) == 1),1); % was in X

intensity_hold = Sorted_NP_Saliva_Data.SampleVol.Neg((Sorted_NP_Saliva_Data.SampleVol.Neg(:,2) == 1),7:8);
avg1min_Cyan_neg_Cyan = [Dilution_hold intensity_hold];

% Extract for positive
Dilution_hold = Sorted_NP_Saliva_Data.SampleVol.Pos((Sorted_NP_Saliva_Data.SampleVol.Pos(:,2) == 1),1);
intensity_hold = Sorted_NP_Saliva_Data.SampleVol.Pos((Sorted_NP_Saliva_Data.SampleVol.Pos(:,2) == 1),7:8);
avg1min_Cyan_Sso_Cyan = [Dilution_hold intensity_hold];
...
SampleVol_plot = linspace(5,50,51); % [Fold Dilution] Probe from 0.5X to 5.5X
...
Paper_params_in = repmat(Paper_params,length(SampleVol_plot),1);

Soln_pptys_in = repmat(Soln_pptys,length(SampleVol_plot),1);
op_params_in = cell(length(SampleVol_plot),3);
...
for i = 1:length(SampleVol_plot) 
    op_params_in{i,2} = [SampleVol_plot(i) 50].*(1e-6)*1e-3; %[m3] 15 uL sample + 50 (25+25) uL wash
end

M = 26; N = 90; %Mesh sizes

% Intialize
Cyan_HRP_model_pos = zeros(size(SampleVol_plot));
Cyan_HRP_model_neg = zeros(size(SampleVol_plot));

Sample_Conc = 5e-9*1e3; % 5nM NP used as baseline.

MW_HRP = Soln_pptys(3)*1000;
parfor i = 1:length(SampleVol_plot)
...
figure;
plot(SampleVol_plot, Cyan_HRP_model_pos,'LineWidth',1.5); 
hold on;
...
plot(SampleVol_plot, Cyan_HRP_model_neg,'LineWidth',1.5); %convert to uL/s, plot in red, Cyan express in percent
...
title('Influence of Sample Volume on Signal generated'); box on;
xlabel('Sample Volume (\mu L)'); ylabel('Cyan Intensity (AU)');
...
saveas(gcf,'rcSso_NP_Saliva_vary_SampleVol_plot.fig')
